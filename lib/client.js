...
window.__ModuleLoader__.load({ id: 'dsh-token-anxiety', factory: (require) => {
  ...
  // fetch with a hard timeout (fire-and-forget persistence calls must never
  // leave an unbounded request hanging).
  ...
  var CSS = [
    ...
    '.txg-hero-v { font-size:28px; font-weight:700; letter-spacing:-.02em; font-variant-numeric:tabular-nums; line-height:1.1; }',
    '.txg-hero-s { font-size:15px; opacity:.8; }',
    '.txg-post { color:#e04848; font-weight:600; }',
    '.txg-stat { display:flex; align-items:baseline; justify-content:space-between; gap:12px; padding:5px 0; }',
    '.txg-stat-l { font-size:11.5px; letter-spacing:.05em; text-transform:uppercase; opacity:.5; flex:none; }',
    '.txg-stat-v { font-size:14.5px; font-variant-numeric:tabular-nums; text-align:right; font-weight:600; }',
    ...
    // pro model: distinct bar palette (blue family vs flash's green/orange).
    '.txg-fill-pro { background:linear-gradient(90deg,#3d7ae0,#8b5cf6); opacity:.85; }',
    '.txg-sg-pro-miss { background:#5b8def; }',
    '.txg-sg-pro-hit { background:#38bdf8; }',
    '.txg-sg-pro-out { background:#c084fc; }',
    '.txg-mlbl { font-size:10px; opacity:.6; margin-top:3px; }',
    '.txg-regime { font-size:11px; font-weight:600; float:right; text-transform:none; letter-spacing:normal; }',
  ...
  function TokenAnxietyWidget(props) {
    ...
    var curState = React.useState(function () { return 'COP'; });
    var cur = curState[0];
    ...
    function modelShort(mn) {
      return mn === 'deepseek-v4-flash' ? 'flash' : (mn === 'deepseek-v4-pro' ? 'pro' : mn);
    }
    function isProModel(mn) { return mn === 'deepseek-v4-pro'; }
    function modelBar(m, maxCop, maxTok, showLabel) {
      var pro = isProModel(m.model);
      var pct = Math.max(2, Math.round((m.cop || 0) / maxCop * 100));
      var mPct = (m.missTokens || 0) / maxTok * 100;
      var hPct = (m.hitTokens || 0) / maxTok * 100;
      var oPct = (m.outputTokens || 0) / maxTok * 100;
      var els = [];
      if (showLabel) els.push(React.createElement('div', { key: 'lbl', className: 'txg-mlbl' }, modelShort(m.model)));
      els.push(React.createElement('div', { key: 'tr', className: 'txg-track' },
        React.createElement('div', { className: 'txg-fill' + (pro ? ' txg-fill-pro' : ''), style: { width: pct + '%' } }, null),
        React.createElement('div', { className: 'txg-num' }, money(m.cop))));
      els.push(React.createElement('div', { key: 'st', className: 'txg-stack' },
        React.createElement('div', { className: pro ? 'txg-sg-pro-miss' : 'txg-sg-miss', style: { width: mPct + '%' } }, null),
        React.createElement('div', { className: pro ? 'txg-sg-pro-hit' : 'txg-sg-hit', style: { width: hPct + '%' } }, null),
        React.createElement('div', { className: pro ? 'txg-sg-pro-out' : 'txg-sg-out', style: { width: oPct + '%' } }, null)));
      return els;
    }
    // Shared cost-graph column: renders one cost track + token stack per model.
    // A task that used both flash and pro shows four bars (flash cost/tokens,
    // pro cost/tokens), pro in its own blue palette.
    function barGraph(t, maxCop, maxTok) {
      var ms = t.models || [];
      if (ms.length > 1) {
        var els = [];
        for (var mi = 0; mi < ms.length; mi++) {
          els = els.concat(modelBar(ms[mi], maxCop, maxTok, true));
        }
        return React.createElement('div', { className: 'txg-bars' }, els);
      }
      var single = ms.length === 1 ? ms[0] : null;
      var agg = single || { model: null, cop: t.cop, missTokens: t.missTokens, hitTokens: t.hitTokens, outputTokens: t.outputTokens };
      return React.createElement('div', { className: 'txg-bars' }, modelBar(agg, maxCop, maxTok, false));
    }
    ...
    if (tasks.length) {
      ...
      // Any shown task with a per-task model bucket that includes pro? Then
      // the per-model bars use the pro palette and the legend adds pro rows.
      var hasProRows = sortedTasks.some(function (t) {
        var tms = t.models || [];
        for (var hpi = 0; hpi < tms.length; hpi++) {
          if (tms[hpi].model === 'deepseek-v4-pro') return true;
        }
        return false;
      });
      var rows = sortedTasks.map(function (t, i) {
        ...
        return React.createElement('div', {
          ...
          React.createElement('div', { className: 'txg-cell-cost', title: money(t.cop) + (showProj ? ' \u2192 ' + money(t.postCop) : '') },
            ...
            showProj ? React.createElement('div', { className: 'txg-post' }, '\u2192 ' + money(t.postCop)) : null),
      ...
      tasksTab = React.createElement('div', { className: 'txg-tasks' },
        React.createElement('div', { className: 'txg-sec' },
          ...
          React.createElement('div', { className: 'txg-hero' },
            ...
            showProj ? React.createElement('span', { className: 'txg-hero-s' },
              ' \u2192 ',
              React.createElement('span', { className: 'txg-post' }, money(conv.postCop))) : null),
        ...
        React.createElement('div', { className: 'txg-legend' },
          ...
          hasProRows ? React.createElement('span', { className: 'txg-lg' }, React.createElement('span', { className: 'txg-sw txg-sg-pro-miss' }, null), L.inputMiss + ' (pro)') : null,
          hasProRows ? React.createElement('span', { className: 'txg-lg' }, React.createElement('span', { className: 'txg-sw txg-sg-pro-hit' }, null), L.cacheHit + ' (pro)') : null,
          hasProRows ? React.createElement('span', { className: 'txg-lg' }, React.createElement('span', { className: 'txg-sw txg-sg-pro-out' }, null), L.output + ' (pro)') : null));
    }
    ...
    // Per-model per-task averages from the per-task model buckets (flash vs
    // pro), so the price-per-task section discriminates by model instead of
    // showing one blended conversation-wide number.
    var perModel = {};
    for (var omi = 0; omi < allTasks.length; omi++) {
      var omt = allTasks[omi];
      var omtModels = omt.models || [];
      for (var omj = 0; omj < omtModels.length; omj++) {
        var omr = omtModels[omj];
        var omk = omr.model;
        var oma = perModel[omk] || { tasks: 0, cop: 0, postValley: 0, postPeak: 0 };
        oma.tasks += 1;
        oma.cop += omr.cop || 0;
        oma.postValley += omr.postValleyCop || 0;
        oma.postPeak += omr.postPeakCop || 0;
        perModel[omk] = oma;
      }
    }
    var modelRowEls = Object.keys(perModel).map(function (omk) {
      var oma = perModel[omk];
      var omn = Math.max(1, oma.tasks);
      var oname = omk === 'deepseek-v4-flash' ? 'flash' : (omk === 'deepseek-v4-pro' ? 'pro' : omk);
      return statRow(oname,
        React.createElement('span', null,
          money(oma.cop / omn),
          showProj ? React.createElement('span', { className: 'txg-post' },
            ' ' + L.postArrow + ' ' + L.heroPostValley.replace('{cost}', money(oma.postValley / omn)) + L.heroPostPeak.replace('{cost}', money(oma.postPeak / omn))) : null));
    });
    // Real average price per task (total / count, NOT the conversation total)
    // plus the regime that applies right now (valley now / peak now), so the
    // hero no longer compares a peak-weighted total against an all-valley one.
    var perTaskNow = conv.taskCount > 0 ? conv.totalCop / conv.taskCount : 0;
    var perTaskValley = conv.taskCount > 0 ? conv.postValleyCop / conv.taskCount : 0;
    var perTaskPeak = conv.taskCount > 0 ? conv.postPeakCop / conv.taskCount : 0;
    var modelUsage = Object.keys(perModel).map(function (omk) {
      var oma = perModel[omk]
      var oname = omk === 'deepseek-v4-flash' ? 'flash' : (omk === 'deepseek-v4-pro' ? 'pro' : omk)
      var share = conv.totalCop > 0 ? Math.round((oma.cop / conv.totalCop) * 1000) / 10 : 0
      return statRow(oname, React.createElement('span', null,
        money(oma.cop),
        React.createElement('span', { className: 'txg-stat-sub' }, ' \u00b7 ' + share + '%')))
    })
    var peakCop = money(round2(regime.peak || 0))
    var valleyCop = money(round2(regime.valley || 0))
    var overview = React.createElement('div', null,
      React.createElement('div', { className: 'txg-sec' },
        React.createElement('div', { className: 'txg-sec-t' },
          L.cost,
          React.createElement('span', { className: 'txg-regime ' + (peak ? 'txg-peak' : 'txg-valley') },
            L.nowBadge.replace('{regime}', peak ? L.peakWord : L.valleyWord))),
        React.createElement('div', { className: 'txg-hero' },
          React.createElement('span', { className: 'txg-hero-v' }, money(conv.totalCop)),
          showProj ? React.createElement('span', { className: 'txg-hero-s txg-post' },
            L.postArrow + ' ' + money(conv.postCop)) : null),
        statRow(L.tasks, String(conv.taskCount)),
        statRow(L.requests, String(totalReq)),
        statRow(L.tokens, 'in ' + fmtTok(totalMiss + totalHit) + ' \u00b7 out ' + fmtTok(totalOut)),
        modelUsage),
      showProj ? React.createElement('div', { className: 'txg-sec' },
        React.createElement('div', { className: 'txg-sec-t' }, L.peakWord + ' / ' + L.valleyWord),
        statRow(L.peakWord, peakCop),
        statRow(L.valleyWord, valleyCop)) : null,
      React.createElement('div', { className: 'txg-sec' },
    ...
    function pricingTableBlock(p) {
      ...
      // No flat 'current' rate anymore: pricing is always peak/valley, so the
      // single live column shows the rate in effect right now.
      var liveRate = peak ? 'peak' : 'valley';
      ...
      var rows = models.map(function (mn) {
        var r = rates[mn];
        if (!r) return null;
        var name = mn === 'deepseek-v4-flash' ? 'flash' : (mn === 'deepseek-v4-pro' ? 'pro' : (mn === 'deepseek-v4-flash-vision-exp' ? 'vision' : mn));
        if (!showProj) {
          ...
          return React.createElement('tr', { key: mn },
            ...
            React.createElement('td', { className: 'txg-px-cur' }, priceCell(r[liveRate])));
        }
        if (postHike) {
        ...
        return React.createElement('tr', { key: mn },
          ...
          React.createElement('td', null,
            projLine(L.peakWord, r.peak, r[liveRate]),
            ' \u00b7 ',
            projLine(L.valleyWord, r.valley, r[liveRate])));
      });
      return React.createElement('div', null,
    ...
    function persistCurrencies(list) {
      fetchWithTimeout('/token-anxiety/currencies', {
        method: 'POST',
