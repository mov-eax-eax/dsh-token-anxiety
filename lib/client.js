...
//
// The only external is react (a platform seed word); everything else is
// inline. The plugin registers one entry in the conversation.input.dock strip
// and renders the Token Anxiety chip from the 'tokenAnxiety' session
...
//
// Explain: the chip's button fetch()es the bundle's own host route
...
window.__ModuleLoader__.load({ id: 'dsh-token-anxiety', factory: (require) => {
  ...
  var CSS = [
    '.txg-chip { display:inline-flex; align-items:center; gap:6px; font-size:11px; line-height:1; padding:3px 9px; border-radius:999px; background:rgba(160,160,160,.14); color:inherit; white-space:nowrap; }',
    '.txg-valley { color:#2ea867; }',
    '.txg-peak { color:#e08b2d; }',
    '.txg-muted { opacity:.72; }',
    '.txg-pop { position:absolute; bottom:calc(100% + 8px); left:0; z-index:60; width:720px; max-width:96vw; height:64vh; max-height:88vh; display:flex; flex-direction:column; overflow:hidden; background:#1b1b1f; color:#eceaf0; border:1px solid rgba(255,255,255,.15); border-radius:10px; padding:12px 14px; font-size:11px; line-height:1.55; box-shadow:0 10px 30px rgba(0,0,0,.5); text-align:left; }',
    ...
    '.txg-tbl-head { display:flex; gap:8px; align-items:center; padding:3px 0; border-bottom:1px solid rgba(255,255,255,.12); font-size:9px; text-transform:uppercase; letter-spacing:.08em; opacity:.7; }',
    ...
    '.txg-tip-fixed { position:fixed; top:auto; left:auto; z-index:90; }',
    ...
    '.txg-tab { flex:1; padding:6px 4px; text-align:center; font-size:10px; font-weight:600; border-radius:6px 6px 0 0; background:transparent; border:none; color:inherit; opacity:.55; cursor:pointer; }',
    ...
    '.txg-tab-n { display:inline-block; margin-left:4px; padding:0 5px; border-radius:999px; background:rgba(229,72,77,.35); color:#ffd7d7; font-size:9px; }',
    ...
    '.txg-lbl { display:flex; justify-content:space-between; font-size:9px; opacity:.55; margin-top:4px; }',
    '.txg-selrow { display:flex; gap:4px; margin-top:4px; flex-wrap:wrap; }',
    '.txg-selbtn { font-size:10px; padding:2px 8px; border-radius:999px; border:1px solid rgba(255,255,255,.22); background:transparent; color:inherit; opacity:.7; cursor:pointer; }',
    ...
    '.txg-cc-panel { position:absolute; top:calc(100% + 4px); left:0; z-index:75; min-width:180px; max-height:280px; overflow-y:auto; background:#26262c; border:1px solid rgba(255,255,255,.18); border-radius:8px; padding:4px; box-shadow:0 8px 24px rgba(0,0,0,.5); }',
    '.txg-cc-pick { display:flex; align-items:center; gap:6px; flex:1; cursor:pointer; min-width:0; }',
    '.txg-cc-x { border:none; background:transparent; color:inherit; opacity:.5; cursor:pointer; font-size:10px; padding:2px 4px; border-radius:4px; }',
    '.txg-cc-x:hover { opacity:1; background:rgba(255,255,255,.15); }',
    '.txg-cc-add { margin-top:4px; padding:5px 8px; border-top:1px solid rgba(255,255,255,.12); font-size:10px; color:#e08b2d; cursor:pointer; }',
    '.txg-cc-addlist { margin-top:4px; border-top:1px solid rgba(255,255,255,.12); padding-top:4px; max-height:170px; overflow-y:auto; }',
    '.txg-cc-search { width:100%; box-sizing:border-box; font-size:10px; padding:4px 6px; border-radius:6px; border:1px solid rgba(255,255,255,.2); background:rgba(255,255,255,.06); color:inherit; margin-bottom:4px; }',
    '.txg-cc-item { display:flex; justify-content:space-between; align-items:center; gap:12px; padding:5px 10px; border-radius:6px; cursor:pointer; font-size:11px; }',
    '.txg-cc-item:hover { background:rgba(255,255,255,.08); }',
    '.txg-cc-check { color:#e08b2d; }',
    '.txg-cc-rate { font-size:9px; opacity:.55; flex:none; margin-left:4px; }',
    '.txg-currow { display:flex; align-items:center; gap:6px; margin-top:2px; }',
    '.txg-projchk { display:inline-flex; align-items:center; gap:4px; font-size:10px; opacity:.75; cursor:pointer; margin-left:auto; }',
    '.txg-cur-lbl { opacity:.6; margin-right:2px; }',
    '.txg-sec { margin-top:12px; }',
    '.txg-sec-t { font-size:9px; font-weight:700; letter-spacing:.14em; text-transform:uppercase; opacity:.55; margin-bottom:6px; padding-bottom:4px; border-bottom:1px solid rgba(255,255,255,.08); }',
    '.txg-hero { display:flex; align-items:baseline; gap:8px; flex-wrap:wrap; margin-bottom:4px; }',
    '.txg-hero-v { font-size:20px; font-weight:650; letter-spacing:-.01em; font-variant-numeric:tabular-nums; }',
    '.txg-hero-s { font-size:14px; opacity:.85; }',
    ...
    '.txg-stat-l { font-size:9px; letter-spacing:.05em; text-transform:uppercase; opacity:.5; flex:none; }',
    '.txg-stat-v { font-size:12px; font-variant-numeric:tabular-nums; text-align:right; }',
    '.txg-stat-sub { font-size:9px; opacity:.6; margin-top:1px; text-align:right; }',
    '.txg-note { opacity:.6; font-size:10px; margin:3px 0; }',
    ...
    '.txg-track { position:relative; height:14px; background:rgba(255,255,255,.07); border-radius:3px; overflow:hidden; }',
    '.txg-fill { position:absolute; top:0; bottom:0; left:0; background:linear-gradient(90deg,#2ea867,#e08b2d); opacity:.85; }',
    '.txg-num { position:absolute; right:4px; top:1px; font-size:10px; }',
    ...
    '.txg-legend { display:flex; gap:10px; margin-top:5px; font-size:10px; opacity:.7; }',
    '.txg-lg { display:inline-flex; align-items:center; gap:4px; }',
    '.txg-sw { display:inline-block; width:8px; height:8px; border-radius:2px; }',
    '.txg-tip { position:absolute; bottom:calc(100% + 4px); left:0; z-index:70; width:max-content; max-width:420px; background:#26262c; border:1px solid rgba(255,255,255,.18); border-radius:8px; padding:20px 10px 8px; font-size:10px; line-height:1.5; box-shadow:0 8px 24px rgba(0,0,0,.5); }',
    '.txg-tip-x { position:absolute; top:2px; left:2px; z-index:3; width:15px; height:15px; line-height:1; border:none; background:transparent; color:inherit; opacity:.55; cursor:pointer; border-radius:4px; font-size:10px; padding:0; }',
    ...
    '.txg-ft { opacity:.55; font-size:10px; margin-top:8px; border-top:1px solid rgba(255,255,255,.1); padding-top:6px; }',
    ...
    '.txg-wbtn { font-size:10px; padding:3px 10px; border-radius:6px; border:1px solid rgba(224,139,45,.6); background:transparent; color:inherit; cursor:pointer; }',
    '.txg-wbtn:hover { background:rgba(224,139,45,.2); }',
    '.txg-wbtn:disabled { opacity:.5; cursor:default; }',

    '.txg-explx { background:transparent; border:none; color:inherit; opacity:.6; cursor:pointer; font-size:11px; }',
    '.txg-explhead { display:flex; align-items:center; justify-content:space-between; gap:8px; margin-top:6px; font-weight:600; }',
    '.txg-expl { max-height:240px; overflow:auto; white-space:pre-wrap; margin-top:2px; padding:6px 8px; border:1px solid rgba(255,255,255,.12); border-radius:6px; background:rgba(255,255,255,.05); font-size:10px; line-height:1.5; }',
    '.txg-bar-row-sel { outline:1px solid rgba(224,139,45,.55); border-radius:4px; }',
    '.txg-px { width:100%; border-collapse:collapse; font-size:10px; }',
    '.txg-px th, .txg-px td { border:1px solid rgba(255,255,255,.1); padding:4px 6px; text-align:left; vertical-align:top; font-variant-numeric:tabular-nums; }',
  ...
  var I18N = {
    en: {
      overview: 'Overview', tasks: 'Tasks', prices: 'Prices',
      ...
      peakInEffect: 'peak/valley pricing in effect since {date}',
      projectedFrom: 'projected from {date} \u00b7 USD per 1M tokens', usdPer1m: 'USD per 1M tokens',
      asOf: 'as of {date} \u00b7 {source}', model: 'Model', current: 'Current', projectedPostHike: 'Projected post-hike',
      fxRatesLine: 'FX: {list}', fxRate: '1 USD = {rate} {code}',
      ...
      reqLine: '{req} req \u00b7 miss {miss} / hit {hit} / out {out}', postSuffix: ' \u00b7 post {cost} ({peak})',
    ...
    zh: {
      overview: '概览', tasks: '任务', prices: '价格',
      ...
      peakInEffect: '高峰/低谷定价自 {date} 起生效',
      projectedFrom: '自 {date} 起预测 · 每百万 token 美元价', usdPer1m: '每百万 token 美元价',
      asOf: '截至 {date} · {source}', model: '模型', current: '当前', projectedPostHike: '预测涨价后',
      fxRatesLine: '汇率：{list}', fxRate: '1 美元 = {rate} {code}',
      peakCol: '高峰', valleyCol: '低谷',
      noRecordedContent: '（无记录内容）', possibleWaste: '疑似浪费：{flags}',
      nowShare: '当前 {cost} · 占对话 {pct}%', postLabel: ' · 预测 {cost}',
      reqLine: '{req} 请求 · 未命中 {miss} / 命中 {hit} / 输出 {out}', postSuffix: ' · 预测 {cost}（{peak}）',
  ...
  // ---- the chip component ----------------------------------------------------
  function TokenAnxietyChip(props) {
    ...
    var showProjState = React.useState(false);
    ...
    function barGraph(t, maxCop, maxTok) {
      ...
      return React.createElement('div', { className: 'txg-bars' },
        ...
        React.createElement('div', { className: 'txg-muted', style: { fontSize: 9 } },
          L.reqLine.replace('{req}', String(t.requests)).replace('{miss}', fmtTok(t.missTokens || 0)).replace('{hit}', fmtTok(t.hitTokens || 0)).replace('{out}', fmtTok(t.outputTokens || 0)) + (showProj ? L.postSuffix.replace('{cost}', money(t.postCop)).replace('{peak}', postPeakStr(t)) : '') + (dshort ? (' \u00b7 ' + dshort) : '')));
    }
    ...
    function doExplain(turn) {
      if (explPending[turn] || explainTextOf(turn)) return;
      setPending(turn, true);
      var p = null;
      try {
        p = onExplain ? onExplain(turn) : null;
      } catch (e) {
      ...
      if (p && typeof p.then === 'function') {
        p.then(function (v) {
          ...
          setPending(turn, false);
        }, function () { setPending(turn, false); });
      } else {
        setPending(turn, false);
      }
    }
    ...
    function explainBlock(turn, label) {
      ...
      if (text && !explDismiss[turn]) {
        return React.createElement('div', null,
          ...
          React.createElement('div', { className: 'txg-expl' }, text));
      }
      if (explPending[turn]) {
        return React.createElement('div', { className: 'txg-muted', style: { marginTop: 3 } }, L.askAnalyzing);
      }
      return null;
    }
    ...
    var overview = React.createElement('div', null,
      ...
      conv.taskCount > 0 ? React.createElement('div', { className: 'txg-sec' },
        React.createElement('div', { className: 'txg-sec-t' }, L.costShape),
        statRow(L.avgPerTask, money(round2(avgTask))),
        statRow(L.avgPerRequest, money(round2(avgReq))),
        mostExp ? React.createElement('div', { className: 'txg-stat' },
          React.createElement('div', { className: 'txg-stat-l' }, L.mostExpensive),
          React.createElement('div', { className: 'txg-stat-v' },
            React.createElement('b', null, '#' + String(mostExp.turn)), ' ', money(mostExp.cop), ' (' + mostExp.sharePct + '%)',
            mostExp.preview ? React.createElement('div', { className: 'txg-stat-sub' }, '\u201c' + mostExp.preview + '\u201d') : null)) : null) : null,
      wasteTotal > 0 ? React.createElement('div', { className: 'txg-sec' },
        React.createElement('div', { className: 'txg-sec-t' }, L.wasteSnapshot),
        statRow(L.flagged, String(wasteTotal) + ' / ' + String(conv.taskCount)),
        statRow(L.cost, '~' + money(wasteCop) + (conv.totalCop > 0 ? (' (' + Math.round(wasteCop / conv.totalCop * 1000) / 10 + '%)') : '')),
        topFlags.length ? statRow(L.topFlags, topFlags.map(function (f) { return wasteLabel[f] || f; }).join(' \u00b7 ')) : null) : null,
      conv.models && conv.models.length
        ? React.createElement('div', { className: 'txg-sec' },
          React.createElement('div', { className: 'txg-sec-t' }, L.models),
          conv.models.map(function (mo) {
            return React.createElement('div', { key: mo.model, className: 'txg-stat' },
              React.createElement('div', { className: 'txg-stat-l' },
                React.createElement('b', null, mo.model === 'deepseek-v4-flash' ? 'flash' : 'pro'),
                ' \u00b7 ' + mo.requests + ' req'),
              React.createElement('div', { className: 'txg-stat-v' },
                money(mo.cop), ' (' + mo.sharePct + '%)',
                React.createElement('div', { className: 'txg-stat-sub' },
                  'miss ' + fmtTok(mo.missTokens || 0) + ' / hit ' + fmtTok(mo.hitTokens || 0) + ' / out ' + fmtTok(mo.outputTokens || 0),
                  showProj ? (' \u00b7 post ' + L.peakWord + ' ' + money(mo.peakCop || 0) + ' / ' + L.valleyWord + ' ' + money(mo.valleyCop || 0)) : null)));
          }))
        : null);
    ...
    function pricingTableBlock(p) {
      ...
      var rows = models.map(function (mn) {
        ...
        return React.createElement('tr', { key: mn },
          React.createElement('td', null, name),
          React.createElement('td', { className: 'txg-px-cur' }, priceCell(r.current)),
          React.createElement('td', { className: 'txg-px-proj' },
            React.createElement('div', null, L.peakWord + ' ' + priceCell(r.peak)),
            React.createElement('div', null, L.valleyWord + ' ' + priceCell(r.valley))));
      });
      return React.createElement('div', null,
        React.createElement('table', { className: 'txg-px' },
          React.createElement('thead', null, React.createElement('tr', null, head)),
          React.createElement('tbody', null, rows)),
        React.createElement('div', { className: 'txg-muted', style: { marginTop: 3, fontSize: 9 } },
          showProj
            ? (postHike
              ? L.peakInEffect.replace('{date}', p.effectiveFromUtc || '')
              : L.projectedFrom.replace('{date}', p.effectiveFromUtc || ''))
            : L.usdPer1m));
    }
    var pricesTab = React.createElement('div', null,
      React.createElement('div', { className: 'txg-sec' },
        React.createElement('div', { className: 'txg-sec-t' }, L.pricing),
        pricingTableBlock(pricingNow),
        React.createElement('div', { className: 'txg-note' }, L.asOf.replace('{date}', pricingNow.asOf || '?').replace('{source}', pricingNow.source || '')),
        React.createElement('div', { className: 'txg-note' }, L.fxRatesLine.replace('{list}', enabledCurrencies.map(function (code) {
          var rt = fxRates[code];
          return rt ? L.fxRate.replace('{rate}', fmtRate(rt)).replace('{code}', code) : null;
        }).filter(Boolean).join(' \u00b7 ')))));

    // ---- chip + popup ----
    var icon = peak ? '\u2600' : '\ud83c\udf19';
    var chip = React.createElement('span', {
      className: 'txg-chip' + (peak ? ' txg-peak' : ' txg-valley'),
    ...
    // Fixed-position tooltip for task rows (escapes the rows scroll container).
    var hoverTip = null;
    if (hovTip && hovTip.task && !tipDismiss['t:' + hovTip.task.turn]) {
      var ht = hovTip.task;
      var hLabel = (isWaste(ht) ? '\u26a0 ' : '') + '#' + String(ht.turn);
      var hDs = fmtDate(ht, tz);
      hoverTip = React.createElement('div', { className: 'txg-tip txg-tip-fixed', style: { bottom: (window.innerHeight - hovTip.top + 4), left: hovTip.left } },
    ...
    var pop = show ? React.createElement('div', { className: 'txg-pop' },
      React.createElement('div', { className: 'txg-top' },
              ...
              enabledCurrencies.map(function (code) {
                ...
                return React.createElement('div', { key: code, className: 'txg-cc-item' },
                  React.createElement('span', {
                    ...
                    React.createElement('span', null, L.flagCurrency.replace('{code}', code).replace('{name}', entry ? entry.name : code)),
              ...
              addOpen ? React.createElement('div', { className: 'txg-cc-addlist' },
                ...
                CURRENCY_CATALOG.filter(function (c) {
                  ...
                  return React.createElement('div', {
                    ...
                    React.createElement('span', null, c.code + ' \u00b7 ' + c.name),
        ...
        React.createElement('div', { className: 'txg-tabs' },
          ...
          React.createElement('button', { className: 'txg-tab' + (tab === 'prices' ? ' txg-tab-on' : ''), onClick: function () { setTab('prices'); } }, L.prices))),
      strip,
    ...
    return React.createElement('span', { style: { position: 'relative', display: 'inline-flex' } }, chip, pop);
  }
      ...
      return ctx.slots.register({
        ...
        inject: function (sessionId) {
          return {
            ...
            onExplain: function (task) {
              // Direct host call: the host half registers /token-anxiety/explain
              // on the webserver. Same-origin fetch; no agent turn, no session event.
