...
window.__ModuleLoader__.load({ id: 'dsh-token-anxiety', factory: (require) => {
  ...
  var CSS = [
    ...
    '.txg-tbl-head { display:flex; gap:8px; align-items:center; padding:3px 0; border-bottom:1px solid rgba(255,255,255,.12); font-size:11px; text-transform:uppercase; letter-spacing:.08em; opacity:.7; }',
    ...
    '.txg-cell-cost { width:190px; flex:none; text-align:right; font-variant-numeric:tabular-nums; font-weight:600; white-space:nowrap; overflow:hidden; text-overflow:ellipsis; }',
    ...
    '.txg-lbl { display:flex; justify-content:space-between; font-size:11px; opacity:.55; margin-top:4px; }',
    ...
    '.txg-sec-t { font-size:11px; font-weight:700; letter-spacing:.14em; text-transform:uppercase; opacity:.55; margin-bottom:6px; padding-bottom:4px; border-bottom:1px solid rgba(255,255,255,.08); }',
    ...
    '.txg-stat-l { font-size:11px; letter-spacing:.05em; text-transform:uppercase; opacity:.5; flex:none; }',
    '.txg-stat-v { font-size:14px; font-variant-numeric:tabular-nums; text-align:right; }',
    '.txg-stat-sub { font-size:11px; opacity:.6; margin-top:1px; text-align:right; }',
    ...
    '.txg-num { position:absolute; right:4px; top:1px; font-size:11px; }',
  ...
  function TokenAnxietyWidget(props) {
    ...
    // Projected peak/valley lines: green when cheaper than the current rate,
    // red when more expensive (compared on the headline miss price).
    function projLine(label, price, cur) {
      ...
      return React.createElement('div', { className: 'txg-px-proj' + extra }, label + ' ' + priceCell(price));
    }
